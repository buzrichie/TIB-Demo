 ...
 * @param errors contains a list of errors in case of a failed API request, could be null if not
 *     applicable
 */
public record ApiResponse<T>(
    ...
    @JsonProperty("errors") @JsonInclude(JsonInclude.Include.NON_NULL) List<T> errors) {

  public static <T> ApiResponse<T> success(T data) {
    return new ApiResponse<>(true, "Success", data, LocalDateTime.now().toString(), null);
  }
  ...
  public static <T> ApiResponse<T> error(String message, List<T> errors) {
    return new ApiResponse<>(false, message, null, LocalDateTime.now().toString(), errors);
  }

  public static <T> ApiResponse<T> error(String message) {
    return new ApiResponse<>(false, message, null, LocalDateTime.now().toString(), null);
  }
}
